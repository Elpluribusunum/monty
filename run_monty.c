 ...
 *
 * Return: length (as an int) of the current op_toks.
 */
unsigned int token_arr_len(void)
{
...
/**
 * is_empty_line - dmin wth a line taken 4r getline solely consist of delimiter
 * @line: an indicator of the line.
 * @delims: delimiter characters in a string.
 *
 * Return: If the line only has delimiters,
 * the number is 1. If not, then 0.
 */
int is_empty_line(char *line, char *delims)
{
	int i, j;
 ...
 *
 * Return: a reference to the appropriate function.
 */
void (*get_op_func(char *opcode))(stack_t**, unsigned int)
{
 ...
 *
 * Return: If successful, EXIT_SUCCESS; if unsuccessful, the approri error code
 */
int run_monty(FILE *script_fd)
{
	stack_t *stack = NULL;
